...
from ragas_evaluator import ragas_evaluation_with_retrieval
...
def ai_evaluator(predictions, answers, lengths, questions, contexts, df):
    final_scores = {
        "ragas": {},
        # "deep-eval": {}
    }
    ...
    for idx, (prediction, ground_truths, length, question, context) in enumerate(
        zip(predictions, answers, lengths, questions, contexts)
    ):
        contexts_list = ast.literal_eval(context)

        best_answer_similarity = 0.0
        result = {}
        for ground_truth in ground_truths:
            tmp_result = ragas_evaluation_with_retrieval(
                question, prediction, ground_truth, contexts_list
            )

            if (
                tmp_result.get("answer_similarity", 0.0) > best_answer_similarity
                or result == {}
            ):
                result = tmp_result
                best_answer_similarity = tmp_result.get("answer_similarity", 0.0)

        results.append(result)

        df.loc[idx, "answer_correctness"] = result.get("answer_correctness", 0.0)
        # Determine the length category
        if length <= 4000:
            length_bin = "0-4k"
        elif length <= 8000:
            length_bin = "4-8k"
...
def evaluate(file_path, dataset_name, is_ai_evaluator=False):
    if os.path.exists(file_path):
        try:
            ...
            df.to_csv(f"results_{file_path}", index=False)
...
if __name__ == "__main__":
    ...
    retriever = "reranker_with_usefulness_prompt"
    evaluate(
    ...
    with open(f"evaluator_rag_{retriever}_results.json", "w") as json_file:
